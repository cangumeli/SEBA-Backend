...
module.exports.handle = ({ endpoint, validation, data }) => async (req, res) => {
  ...
  try {
    ...
    if (!err.responseCode) {
      // Error is not deliberately created
...
module.exports.validateBody = ({ validation, data }) => (req, res, next) => {
  ...
  const validateBodyRecursive = (validation, body) => {
    // returns whether or not to call next
...
module.exports.refinedMongooseSchema = function refinedMongooseSchema(Model) {
  const obj = Model.schema.paths;
  const refinedSchema = {};
  Object.keys(obj).forEach(key => {
    if (!key.startsWith('_')) {
      let type = obj[key].instance.toLowerCase();
      if (type == 'mixed') {
        type = 'object';
      ...
      refinedSchema[key] = {
        type,
        required: obj[key].options && obj[key].options.required,
      };
    }
  });
  refinedSchema.id = {
