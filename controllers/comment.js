    ...
    endpoint({ body: {text, rating, date, upvote, downvote, shopId, itemId}, payload }) {
        ...
        if(shopId){
            comment = new ShopComment({
                userId: payload.id, text, rating, date, upvote, downvote, shopId
            });
        }
        else if(itemId){
            comment = new ItemComment({
                userId: payload.id, text, rating, date, upvote, downvote, itemId
            });
        }
        return comment.save();
    },
...
const update = {
    ...
    async endpoint({ body, payload }) {
        const comment = await Comment.findOneAndUpdate({_id: body.id, userId: payload.id}, body, {new: true});
...
const deleteComment = {
    ...
    async endpoint({ body, payload }) {
        const comment = await Comment.findOneAndRemove({_id: body.id, userId: payload.id});
        apiService.errorIf(!comment, apiService.errors.NOT_FOUND, 'NoSuchComment');
